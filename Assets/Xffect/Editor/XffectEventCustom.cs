...
public class XffectEventCustom : Editor 
{
	...
	void DrawCameraShake()
	{
		...
		XEditor.DrawInfo("note this event will change its camera's local position directly.");
	...
	void DrawCheckGlowPerObjMaterial()
	{
		...
		foreach(Transform child in parent)
		{
			...
			if (el == null || el.Material == null)
				continue;
			if (el.Material.shader.name.Contains("Xffect/glow_per_obj"))
			{
				return;
			}
		...
		XEditor.DrawInfo("can't find GlowPerObj shader in this Xffect Obj, you might need to assign a shader in 'Xffect/glow_per_obj' category to one of this obj's EffectLayer.");
	}
	...
	void DrawMainConfig()
	{
		...
		XEditor.DrawFloat("end time(-1 infinite):","",EndTime);
