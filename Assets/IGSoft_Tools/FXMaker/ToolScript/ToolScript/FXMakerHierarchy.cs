...
public class FXMakerHierarchy : MonoBehaviour
{
	...
	void Update()
	{
		...
		// ShotKey - Delete
		if (FXMakerMain.inst.GetFocusInputKey(FXMakerLayout.GetWindowId(FXMakerLayout.WINDOWID.EFFECT_HIERARCHY)) == KeyCode.Delete)
		{
...
// 				gameObject.animation.clip
		...
		for (int n = 0; n < drawTrans.GetChildCount(); n++)
		{
			// Draw vertical line
...
//			GUI.Box(boxRect, new GUIContent(caption, FXMakerMain.HoverCommand_Hierarchy(selObj)), style);
	...
	{
		NcEffectBehaviour.AdjustSpeedEditor(target, fSpeedRate);
	}
